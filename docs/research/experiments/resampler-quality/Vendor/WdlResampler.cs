// ClipShift's vendored copy of NAudio's WdlResampler.
//
// =========================== ALTERED SOURCE VERSION ============================
// This is an ALTERED version of the original software, plainly marked as such per
// clause 2 of the WDL licence reproduced below. Provenance:
//
//   NAudio.Core/Dsp/WdlResampler.cs, fetched 2026-08-18 from
//   https://raw.githubusercontent.com/naudio/NAudio/master/NAudio.Core/Dsp/WdlResampler.cs
//   NAudio is MIT licensed; the class is a C# port of the Cockos WDL resampler,
//   whose zlib-style licence follows below.
//
// ClipShift's modifications, each marked inline with a "CLIPSHIFT:" comment:
//
//   1. Namespace changed from NAudio.Dsp to ClipShift.Audio.Resampling.
//   2. Every Array.Resize of the input buffer m_rsinbuf replaced with a grow-only
//      high-water-mark allocation (EnsureRsinbufCapacity). The original resizes
//      m_rsinbuf on EVERY ResamplePrepare call, and under a continuously varying
//      ratio the requested length jitters, so it allocates on essentially every
//      call. ClipShift's one hard performance constraint is zero managed
//      allocation on the capture hot path.
//   3. BuildLowPass rebuilt the windowed-sinc table whenever the requested cutoff
//      differed from the last one by even one ULP -- thousands of sin/cos calls
//      per call once the ratio is drift-corrected. Now gated on a relative
//      FilterRebuildEpsilon (default 1e-4; set to 0 for the original behaviour).
//   4. Diagnostic counters (RsinbufGrowths, FilterRebuilds, RsinbufCapacity) so a
//      test can assert the steady state is allocation-free.
//
// The resampling arithmetic itself is untouched. See the README next to this file
// for the argument that 2 and 3 are behaviour-preserving.
// ===============================================================================
...
namespace ClipShift.Audio.Resampling
{
    /// <summary>
    ...
    public class WdlResampler
    {
        ...
        // CLIPSHIFT: ---------------------------------------------------------------
        // Diagnostics. A steady-state run must leave RsinbufGrowths and FilterRebuilds
        // unchanged; that is the allocation assertion.

        /// <summary>Number of times the input buffer has had to grow.</summary>
        public int RsinbufGrowths { get; private set; }

        /// <summary>Number of times the windowed-sinc table has been rebuilt.</summary>
        public int FilterRebuilds { get; private set; }

        /// <summary>Current capacity, in samples, of the input buffer.</summary>
        public int RsinbufCapacity => m_rsinbuf.Length;

        /// <summary>
        /// Relative change in the requested low-pass cutoff below which the sinc table
        /// is NOT rebuilt. 0 restores the original per-ULP behaviour.
        /// </summary>
        public double FilterRebuildEpsilon { get; set; } = 1e-4;

        /// <summary>
        /// CLIPSHIFT: grow-only replacement for Array.Resize on m_rsinbuf.
        /// Growth preserves existing content exactly as Array.Resize does; the buffer is
        /// never shrunk, because nothing downstream reads m_rsinbuf.Length -- the valid
        /// extent is tracked by m_samples_in_rsinbuf and rsinbuf_availtemp.
        /// </summary>
        private void EnsureRsinbufCapacity(int required)
        {
            if (m_rsinbuf.Length >= required) return;

            // Grow by half again on top of the request so a slowly drifting ratio settles
            // on a high-water mark instead of reallocating each way it moves.
            int newSize = required + required / 2 + 16;
            var grown = new WDL_ResampleSample[newSize];
            Array.Copy(m_rsinbuf, grown, m_rsinbuf.Length);
            m_rsinbuf = grown;
            RsinbufGrowths++;
        }
        // CLIPSHIFT: ---------------------------------------------------------------
        ...
        public int ResamplePrepare(int out_samples, int nch, out WDL_ResampleSample[] inbuffer, out int inbufferOffset)
        {
            ...
            if (hfs > 1 && m_samples_in_rsinbuf < hfs - 1)
            {
                ...
                {
                    // CLIPSHIFT: was "m_rsinbuf = new WDL_SincFilterSample[m_samples_in_rsinbuf * nch]".
                    // The original discards any prior content and hands back a zeroed buffer;
                    // clearing in place is identical, and allocates nothing once warm.
                    EnsureRsinbufCapacity(m_samples_in_rsinbuf * nch);
                    Array.Clear(m_rsinbuf, 0, m_samples_in_rsinbuf * nch);
                }
            }
            ...
            // CLIPSHIFT: was a per-call Array.Resize to exactly (avail + sreq) * nch,
            // followed by a "did the resize actually give me what I asked for" check and a
            // halve-and-retry. That check derived the usable size from m_rsinbuf.Length,
            // which a grow-only buffer over-reports, so the logical size is used directly.
            // The retry path was already unreachable on .NET: Array.Resize either allocates
            // the full request or throws OutOfMemoryException.
            EnsureRsinbufCapacity((m_samples_in_rsinbuf + sreq) * nch);
        ...
        public int ResampleOut(WDL_ResampleSample[] outBuffer, int outBufferIndex, int nsamples_in, int nsamples_out, int nch)
        {
            ...
            if (nsamples_in < m_last_requested) // flush out to ensure we can deliver
            {
                ...
                // CLIPSHIFT: Array.Resize -> grow-only. The length test the original used to
                // decide whether the pad had been applied is now always satisfied.
                EnsureRsinbufCapacity(alloc_size);
        ...
        private void BuildLowPass(double filtpos)
        {
            ...
            // CLIPSHIFT: the original condition is "m_filter_ratio != filtpos", an exact
            // double comparison. ResampleOut calls this on every pull with a cutoff derived
            // from the live ratio, so a drift-corrected ratio rebuilds the whole
            // Blackman-Harris windowed-sinc table -- wantsize * oversize sin/cos pairs -- on
            // every call. The cutoff only needs to track the ratio to within the stopband's
            // own precision; FilterRebuildEpsilon (relative) sets the threshold.
            bool cutoffMoved = (m_filter_ratio < 0.0 || FilterRebuildEpsilon <= 0.0)
                ? m_filter_ratio != filtpos
                : Math.Abs(filtpos - m_filter_ratio) > FilterRebuildEpsilon * Math.Abs(filtpos);
                ...
                m_lp_oversize != wantinterp)
            {
                ...
                // CLIPSHIFT: grow-only. allocsize is fixed by SetMode, so this allocates
                // exactly once per mode, at init, and never on the hot path.
                if (m_filter_coeffs.Length < allocsize)
                    m_filter_coeffs = new WDL_SincFilterSample[allocsize];
        ...
        // CLIPSHIFT: initialised to empty rather than left null, so the high-water-mark
        // helper below can always read .Length without a null check.
        private WDL_ResampleSample[] m_rsinbuf = Array.Empty<WDL_ResampleSample>(); // WDL_TypedBuf<WDL_ResampleSample>
        private WDL_SincFilterSample[] m_filter_coeffs = Array.Empty<WDL_SincFilterSample>(); // WDL_TypedBuf<WDL_SincFilterSample>
